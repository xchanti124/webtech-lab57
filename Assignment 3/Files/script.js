...
const archiveUrl = "http://localhost:3000";
...
const editArchiveItem = async (
    ...
    return await sendRequest("PUT", `${archiveUrl}/media/${itemId}`, {
        name,
        poster,
        year,
        genre,
...
const deleteArchiveItem = async (itemId) => {
    return await sendRequest("DELETE", `${archiveUrl}/media/${itemId}`);
};
...
const render = async () => {
    ...
    for (const item of filteredArchive) {
        ...
        tableEditElement.classList.add("item-action");
        ...
        const tableDeleteElement = document.createElement("a");
        tableDeleteElement.innerHTML = "(delete)";
        tableDeleteElement.classList.add("item-action");
        tableDeleteElement.href = `#`;
        tableDeleteElement.addEventListener("click", (e) => {
            e.preventDefault();

            if (confirm(`Are you sure you want to delete ${item.name}?`)) {
                deleteArchiveItem(item.id).then(render);
            }
        });

        nameColumnElement.appendChild(tableDeleteElement);
